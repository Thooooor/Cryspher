...
class SatDataset:
    ...
    def deg(self):
        ...
        for i, data in enumerate(self.dataset):
            try:
                self.degree_list.append(utils.degree(data.edge_index[1], num_nodes=data.num_nodes))
            except:
                print("Error computing degree for graph {}".format(i))
        return torch.cat(self.degree_list)
