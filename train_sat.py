...
from data import CrystalDataset, SatDataset, SUB_DATASETS
...
def main():
    ...
    load_subset = True if args.subset is True and args.dataset in SUB_DATASETS else False
    dataset = CrystalDataset(args.dataset, args.data_dir, args.data_file, args.train_ratio, 
                             args.valid_ratio, args.test_ratio, args.batch_size, load_subset)
    ...
    if args.abs_pe and args.abs_pe_dim > 0:
        print("Using absolute position encoding: {}".format(args.abs_pe))
        abs_pe_method = POSENCODINGS[args.abs_pe]
        abs_pe_encoder = abs_pe_method(args.abs_pe_dim, normalization='sym')
        if abs_pe_encoder is not None:
            abs_pe_encoder.apply_to(train_data)
            abs_pe_encoder.apply_to(valid_data)
            
    deg = torch.cat([
        degree(data.edge_index[1], num_nodes=data.num_nodes) for data in train_data])
